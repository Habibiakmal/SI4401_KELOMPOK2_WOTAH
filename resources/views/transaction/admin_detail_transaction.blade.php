                    ...
                    <form method="GET">
                        <div class="row g-3">
                            <div class="col-xl-2 col-md-3 col-sm-4">
                                <a href="/admin/transaction" class="btn btn-secondary add-btn rounded-pill"><i class="ri-add-line align-bottom me-1"></i> Kembali</a>
                            </div>
                            <div class="col-xl-10 col-md-9 col-sm-8">
                                <table width="100%">
                                    <tr>
                                        ...
                                        <td class="text-end">
                                            @if($transaction->status == 'verify' || $transaction->status == 'completed')
                                                <button type="button" class="btn btn-primary rounded-pill" data-bs-toggle="modal" id="create-btn" data-bs-target="#paymentProof"><i class="ri-add-line align-bottom me-1"></i>{{ $transaction->status == 'verify' ? 'Verifikasi' : 'Lihat Bukti ' }} Pembayaran</button>
                                            @endif
                                        </td>
                                    </tr>
                                </table>
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-body">
                        <table class="table">
                            <tr>
                                <th style="width:30%" class="bg-light">Penggunaan Air (m3)</th>
                                <td> {{ $bill->usage }} m3</td>
                            </tr>
                            <tr>
                                <th style="width:20%" class="bg-light">Biaya Beban Tetap Air (Rp)</th>
                                <td> {{ formatRp($bill->amount_per_usage) }}</td>
                            </tr>
                            <tr>
                                <th style="width:20%" class="bg-light">Total Beban Biaya Penggunaan Air (Rp)</th>
                                <td> {{ formatRp($bill->amount_per_usage * $bill->usage) }}</td>
                            </tr>
                            <tr>
                                <th style="width:20%" class="bg-light">Biaya Pemeliharan Meter (Rp)</th>
                                <td> {{ formatRp($bill->maintenance_price) }}</td>
                            </tr>
                            <tr>
                                <th style="width:20%" class="bg-light">PPN 10% (Rp)</th>
                                <td> {{ formatRp($bill->ppn_price) }}</td>
                            </tr>
                            <tr>
                                <th style="width:20%" class="bg-light">Total Biaya (Rp)</th>
    ...
    @elseif($transaction->type == 'service')
        <div class="row mt-4">
            <div class="col-12">
                <div class="card">
                    <div class="card-body">
                        <form method="POST" action="/admin/transaction/{{$transaction->id}}/submit_service">
                            @csrf
                            <table class="table">
                                <tr>
                                ...
                                <tr>
                                    ...
                                    @if($transaction->status == 'pending')
                                        <td>
                                            <textarea class="form-control" required autocomplete="off" name="technition_note" placeholder="Ketik catatan keusakan aktual disini..."></textarea>
                                        </td>
                                    @else
                                ...
                                <tr>
                                    ...
                                    @if($transaction->status == 'pending')
                                        <td>
                                            <input type="text" class="form-control" required autocomplete="off" name="amount" placeholder="0">
                                        </td>
                                    @else
                ...
                <div class="modal-header rounded-border bg-light p-3">
                    ...
                    <div class="modal-body pt-0">
                        ...
                        @if($transaction->status == 'verify')
                            <div class="row mt-3">
                                <div class="col-md-12">
                                    <a onclick="return confirm('Apakah anda yakin menyetujui pembayaran ini ?')" class="btn btn-success" href="/admin/transaction/{{$transaction->id}}/confirm_payment">SETUJUI PEMBAYARAN</a>
                                </div>
                            </div>
                        @endif
                    </div>
